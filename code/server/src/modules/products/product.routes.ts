...
import { authMiddleware, purchaseMiddleware, AuthRequest } from '../../shared/middlewares/auth.middleware.js'
...
router.post('/purchase', authMiddleware, purchaseMiddleware, (req: AuthRequest, res: Response) => {
  ...
  if (!user || user.balance < product.price) return res.status(400).json({ error: 'Insufficient balance' })
